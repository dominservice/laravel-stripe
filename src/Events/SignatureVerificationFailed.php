...
namespace Dominservice\LaraStripe\Events;
...
class SignatureVerificationFailed implements Arrayable
{
    ...
    {
        if (!$secret = config("stripe.webhooks.signing_secrets.{$this->signingSecret}")) {
            throw new RuntimeException("Webhook signing secret does not exist: {$this->signingSecret}");
        }
